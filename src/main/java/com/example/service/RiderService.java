...
public class RiderService {
	/* ================================================ */
	// VARIABLES
	// riderRepo : permite acceder al repositorio de Riders y hacer consultas a la
	// BBDD. Solo lo conoce esta clase
	@Autowired
	private RiderRepository riderRepo;
	// restaurantServ : permite acceder al servicio de restaurantes que les
	// proporcionará los pedidos disponibles a través del pedidosRepo
	@Autowired
	private RestauranteService restaurantServ;
	// checkSecurity : variable que accede a métodos auxiliares para controlar la
	...
	/*
	 * Este método se encarga de guardar riders, ya sea para modificar sus datos o
	 * registrarlos. Controla excepciones de contraseñas, emails y datos ya en uso
	 * 
	 */
	public Rider saveRider(Rider rider)
			throws YaEnUsoException, ContraseniaIncorrectaException, InvalidEmailException, IlegalNumberException {
	...
	/*
	 * Método que hace una consulta por email al repositorio para sacar un rider con
	 * dicho email
	 */
	...
	public void updateForm(Rider rider)
			throws IlegalNumberException, IncompleteFormException, ContraseniaIncorrectaException {

		if (rider.getNif().equals("") || rider.getNombre().equals("") || rider.getCarnet().equals("")
				|| rider.getMatricula().equals("") || rider.getTipoVehiculo().equals("")
				|| rider.getTelefono().equals("") || rider.getEmail().equals("") || rider.getContrasenia().equals(""))
		...
		if ((rider.getContrasenia().length() != 60) && (!rider.getContraseniaDoble().isEmpty())) {
			checkSecurity.equalPass(rider.getContrasenia(), rider.getContraseniaDoble());
			rider.setContrasenia(checkSecurity.cifradoContrasenia(rider.getContrasenia()));
			rider.setContraseniaDoble(checkSecurity.cifradoContrasenia(rider.getContraseniaDoble()));
		}
	...
	/*
	 * Método que actualiza los intentos de un rider. Se suele llamar en login pero
	 * se puede usar a mano
	 */
